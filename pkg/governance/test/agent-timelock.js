...
describe("AgentTimelock", function () {
    ...
    beforeEach(async () => {
        ...
        await hre.deployments.fixture(['MockGateway', 'MockGnosisSafe']);

        let addresses = await hre.ethers.getSigners();
        account = addresses[0];
        ...
        gateway = await hre.ethers.getContract('MockGateway');
        gnosisSafe = await hre.ethers.getContract('MockGnosisSafe');
        exchange = await hre.ethers.getContract('MockContract')
    ...
    describe('axelar disabled: execute', ()=>{
        ...
        beforeEach(async ()=>{
            ...
            timelock = await hre.ethers.getContractAt(TIMELOCK_ABI, timelock.address);

            commandId = hre.ethers.encodeBytes32String('test');
            sourceChain = "10";
            sourceAddress = secondUser.address;
            payload = hre.ethers.utils.defaultAbiCoder.encode(['uint256', 'address'], [0, ZERO_ADDRESS]);
        });
    ...
    describe('axelar enabled: execute', () => {
        ...
        it('Action: SET_NEW_AGENT', async () => {
            ...
            payload = hre.ethers.utils.defaultAbiCoder.encode(['uint256', 'address'], [0, secondUser.address]);
        ...
        it('Action: UPGRADE_TIMELOCK', async () => {
            ...
            payload = hre.ethers.utils.defaultAbiCoder.encode(['uint256', 'address'], [1, secondUser.address]);
            let tx = await (await gateway.send(commandId, sourceChain, account.address, payload, timelock.address)).wait();
        ...
        it('revert: Transaction reverted without a reason string', async () => {
            await gateway.setValidate(true);
            payload = hre.ethers.utils.defaultAbiCoder.encode(['uint256', 'address'], [2, secondUser.address]);
    ...
    describe('upgradeTo', ()=>{
        ...
        it('upgrade by axelar', async ()=>{
            ...
            let commandId = hre.ethers.encodeBytes32String('test');
            let sourceChain = "10";
            let sourceAddress = account.address;
            let payload = hre.ethers.utils.defaultAbiCoder.encode(['uint256', 'address'], [1, newImplementation]);
    ...
    describe('schedule', () => {
        ...
        let value = 0;
        let data = 0;
        let predecessor = hre.ethers.encodeBytes32String("12");
        let salt = hre.ethers.encodeBytes32String("32");
        let delay = 0;
    ...
    async function deployTimelock(gateway, motherTimelock, ovnAgent) {
        ...
        timelock = await hre.ethers.getContract('AgentTimelock')
    }
