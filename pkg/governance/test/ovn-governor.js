...
describe("OvnGovernor", function () {
    ...
    sharedBeforeEach('shareBeforeEach', async () => {
        ...
        await hre.deployments.fixture(['Ovn', 'OvnGovernor', 'OvnTimelock', 'OvnTimelockSetting']);
        ...
        ovnToken = await hre.ethers.getContract('Ovn');
        governator = await hre.ethers.getContract('OvnGovernor');
        governator = await hre.ethers.getContractAt(GOVERNOR_ABI, governator.address);

        timeLock = await hre.ethers.getContract('OvnTimelock');
    ...
    async function createMockProposal() {
        ...
        await hre.ethers.provider.send('evm_mine'); // wait 1 block before opening voting
