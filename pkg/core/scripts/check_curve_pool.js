...
async function main() {
    ...
    await execTimelock(async (timelock) => {
        ...
        let CurvePool = await hre.ethers.getContractAt(CurvePoolAbi, curvePoolAddress);

        await (await exchange.setPayoutTimes(1637193600, 24 * 60 * 60, 15 * 60)).wait();
        if (process.env.stand === "arbitrum") {
            let BlackHoleArb = await hre.ethers.getContractAt(HedgeExchangerAbi, "0xe2fe8783CdC724EC021FF9052eE8EbEd00e6248e");
            let MuArb = await hre.ethers.getContractAt(HedgeExchangerAbi, "0xa41b1FDe9495051CaE6e2C98f9Ff763335f75cf0");
